...
2. Afterward, the first `100` assignments from each specified course are uploaded to the database
   ...
   - Names containing "Exam", "Midterm", or "Final" are marked with 🎯, "Project" with 🌀, "Quiz" with 📗, and otherwise with ✏️ 
...
2. The next few sections will show you how to acquire each variable's value 
...
| Variable Name | Value |
| ------------- | ----- |
|CANVAS_ORGANIZATION_TITLE|your organization's version of Canvas Instructure|
|CANVAS_API_KEY|your API key generated for your Canvas account|
|COURSE_ID_LIST|list of canvas course ID's, each separated by a comma|
|NOTION_API_KEY|your API key generated for your Notion account|
...
#### Canvas Variables
1. The organization title can be found by logging in to your Canvas account and inspecting the URL. The highlighted portion is the value of `CANVAS_ORGANIZATION_TITLE` in the `.env` file
   > <img src="readme-images/organization-title-url.PNG" alt="organization title URL" width="300"/>
...
3. The course Id's can be found by visiting each course's homepage and inspecting the URL. The highlighted portion is the class's Canvas course Id. Do this for every class and write out each value separated by a comma in `COURSE_ID_LIST` in the `.env` file
...
3. The Notion database Id can be found by inspecting the URL of your Notion database. For this step, you will have to use Notion's web browser version. The highlighted portion is your database's Id (the portion after "notion.so/" and before the "?"). This is the value of `NOTION_DATABASE_ID` in the `.env` file
   > <img src="readme-images/database-id-url.PNG" alt="database id URL" width="600"/>

4. The name of your checkbox column is the value of `NOTION_CHECKBOX_ID` in the `.env` file
